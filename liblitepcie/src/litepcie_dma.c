...
void litepcie_dma_process(struct litepcie_dma_ctrl *dma)
{
    ...
    if (dma->zero_copy) {
        ...
        {
            if (!ReadFile(dma->dma_fd, dma->buf_rd, DMA_BUFFER_TOTAL_SIZE, &len, NULL))
            {
    ...
    if (dma->zero_copy) {
        ...
        {
            dma->buffers_available_read = DMA_BUFFER_COUNT / 2;
        }
        ...
        dma->buffers_available_write = (dma->reader_sw_count - dma->reader_hw_count);
        if (dma->buffers_available_write > 1)
        {
            if(!WriteFile(dma->dma_fd, dma->buf_wr, DMA_BUFFER_TOTAL_SIZE, &len, NULL))
            {
